...
export default function TestDataPage() {
  ...
  const loginAs = async (email: string, desiredPassword = 'admin123') => {
    try {
      ...
      await fetch('/api/auth/set-password', {
        ...
        body: JSON.stringify({ email, password: desiredPassword })
      }).catch(() => {})

      // Sign in with credentials and go to dashboard
      ...
      // Fallback: try direct sign-in without set-password
  ...
  const runAllTests = async () => {
    ...
    await runTest('Test Weekly Update Email', async () => {
      ...
      if (!usersResponse.ok || !usersData.users?.length) {
        throw new Error('No users found for testing')
      }
      
      const userId = usersData.users[0].id
      const response = await fetch('/api/emails/weekly-update', { 
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          userId: userId,
    ...
    await runTest('Test STG Reminder Email', async () => {
      ...
      const userId = usersData.users[0].id
      const response = await fetch('/api/emails/stg-reminder', { 
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          userId: userId,
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="p-4 border rounded-lg bg-green-50">
                ...
                <div className="mt-3">
                  <Button size="sm" className="bg-green-600 hover:bg-green-700" onClick={() => loginAs('michael.rodriguez@miv.org', 'manager123')}>Login as Manager</Button>
                </div>
              </div>
              ...
              <div className="p-4 border rounded-lg bg-purple-50">
                ...
                <div className="mt-3">
                  <Button size="sm" className="bg-purple-600 hover:bg-purple-700" onClick={() => loginAs('aisha.patel@miv.org', 'analyst123')}>Login as Analyst</Button>
                </div>
              </div>
              ...
              <div className="p-4 border rounded-lg bg-orange-50">
                ...
                <div className="mt-3">
                  <Button size="sm" className="bg-orange-600 hover:bg-orange-700" onClick={() => loginAs('james.thompson@miv.org', 'facilitator123')}>Login as Facilitator</Button>
                </div>
              </div>
              ...
              <div className="p-4 border rounded-lg bg-pink-50">
                ...
                <div className="mt-3">
                  <Button size="sm" className="bg-pink-600 hover:bg-pink-700" onClick={() => loginAs('maria.santos@investor.com', 'stakeholder123')}>Login as Stakeholder</Button>
                </div>
              </div>
              ...
              <div className="p-4 border rounded-lg bg-gray-50">
                ...
                <div className="mt-3">
                  <Button size="sm" className="bg-gray-700 hover:bg-gray-800" onClick={() => loginAs('test@example.com', 'test123')}>Login as Test User</Button>
                </div>
              </div>
            </div>
          </CardContent>
